...
def handle_return(request, pk):
    ...
    # Get the active ReturnRequest for this order
    ...
    # ── reject ────────────────────────────────────────────────────────────────
        ...
        return_req.save()
        # Revert order and items back to delivered
        order.status = 'delivered'
        order.save()
        return_req.items.all().update(return_status='return_rejected')
        messages.warning(request, f"Return #{return_req.id} rejected for Order #{order.id}.")
    ...
    # ── schedule_pickup ───────────────────────────────────────────────────────
        ...
        messages.success(request, f"Pickup scheduled for Return #{return_req.id}.")
    ...
    # ── mark_received ─────────────────────────────────────────────────────────
        ...
        messages.info(request, f"Return #{return_req.id} marked as received at warehouse.")
    ...
    # ── inspection_fail ───────────────────────────────────────────────────────
        ...
        messages.warning(request, f"Inspection failed for Return #{return_req.id}.")
    ...
    # ── mark_refunded ─────────────────────────────────────────────────────────
        ...
        # Close the order as returned
        order.status = 'returned'
        order.save()
        messages.success(request, f"Return #{return_req.id} fully refunded. Order #{order.id} marked as returned.")

    else:
        messages.error(request, f"Invalid action '{action}' for current return status '{return_req.status}'.")
...
def handle_return_item(request, item_id):
    ...
    if action == 'approve':
        if item.return_status in ('return_requested', 'exchange_requested'):
            ...
            item.save()
            messages.success(request, f"Approved return for '{item.product.product_name}'.")
        ...
        if item.return_status in ('return_requested', 'exchange_requested'):
            ...
            messages.warning(request, f"Rejected return for '{item.product.product_name}'.")
        ...
        if item.return_status == 'inspection_passed':
            ...
            item.save()
            messages.success(request, f"Refund ₹{item.refund_amount} initiated.")
